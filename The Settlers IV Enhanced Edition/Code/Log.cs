    ...
    struct Log
    {
        ...
        public static void LogWriter(string logModule, string logMessage)
        {
            if (App.DebugFlag)
            {
                ...
                {
                    w.WriteLine($"{DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss:fffffff", System.Globalization.CultureInfo.InvariantCulture)} ' {logModule.PadRight(25)[..25]} ' {(logMessage.Length > 100 ? logMessage[..100] : logMessage)}");
                }
            }
        }
    }
